...
def kat_3(name):
    data = {
        ...
        "CantLetGo": [
            f'{name}',
            'Гейм-плэй',
            'Начало уровня требует высокой точности от игрока, которому предстоит перепрыгнуть через несколько скоплений шипов, а также придётся столкнуться с обманными сферами, из-за которых придётся подбирать правильный путь. Здесь можно встретить первую монетку в самом низу, над шипами, где вас подкинут три жёлтых батута. Как только фон меняется на чёрный (аналогичный этап встречается в уровне Base After Base), игрока кидает в гущу действий, состоящих из прыжков и скольжений, но, к счастью, эта часть немного легче, чем начальная и длится не так долго. После успешного прохождения «бурной» секции, игрок попадает в режим корабля и преодолевает весьма лёгкий участок уровня. Здесь есть вторая секретная монетка. После этого плавно летим то вверх, то вниз. Однако есть шанс столкнуться со стеной на выходе, поэтому будьте осторожнее! Следом следует тайминговая комбинация из антигравитационного поля и сфер. Там есть третья секретная монетка, с помощью которой можно пропустить некоторые сложные действия. Поле кончается ближе к концу уровня, где игроку придётся преодолеть несколько каверзных прыжков с шипами. Уровень пройден!',
            'Секретные монеты',
            [
                'Первая секретная монета находится на 26% на этапе куба. Для её получения необходимо скатиться со второй из трёх маленьких платформ.',
                'Вторая секретная монета находится на 62% на этапе корабля. Для её получения необходимо пролететь над препятствием с шипами, расположенным близко к потолку.',
                'Третья секретная монета находится на 76% на этапе куба. Для её получения необходимо после смены гравитации и трёх желтых сфер скатиться на один куб и спрыгнуть со следующего.'
            ],
            [
                '../../static/img/lavels/Cantletgo/CLG-C1.png',
                '../../static/img/lavels/Cantletgo/CLG-C2.png',
                '../../static/img/lavels/Cantletgo/CLG-C3.png'
            ]
        ],
        'Jumper': [
            f'{name}',
            'Гейм-плэй',
            'Уровень начинается с несложного паркура по столбам, затем нам нужно будет прыгнуть на батуте и прыгать через шипы, после чего прыгнуть по жёлтым сферам. Снова паркур. На этот раз он будет с другой гравитацией, и сложность его заключается в том, что нужно понять, когда прыгнуть, а когда отпустить. Гравитация становится нормальной. Немного прыгайте и нажмите на сферу. Затем нужно два раза прыгнуть по полублокам через шипы до следующего этапа. Этап корабля. Сначала коридор пустой, только потолок с полом заполнены шипами, но впереди портал гравитации. Летите также как и до этого. Снова нормальная гравитация, но уже со столбами и шипами, которые нужно облететь. На перевёрнутом корабле есть первая монета. Но взять её будет трудно. Она находится под вторым столбом у самого потолка. Затем опять начинается паркур. Там будет вторая монета. Чтобы её собрать, нужно прыгнуть два раза после третьего шипа, затем опять прыгнуть, и отпустить. Потом надо прыгать через шипы и идти через портал. Далее нужно прыгать через шипы, и не прыгнуть лишний раз, так как "потолок" очень низкий. Дойдя до следующего портала не нужно прекращать прыгать. И только после двух шипов отпустить и продолжать прыгать. После следующих двух порталов нужно прыгать по платформам на столбы. На корабле нам предстоит пролететь разбросанные по всему коридору шипы. Там же находится последняя монета и блок с шипом. Для её сбора нужно полететь вниз, когда будут находиться рядом два шипа на одинаковой высоте. Далее облететь оставшиеся шипы. Далее опять следует паркур, и в конце находится 3 шипа, будьте осторожнее. Затем нужно нажать на жёлтую сферу и прыгнуть на батут, затем еще на две сферы. Снова перевёрнутый паркур. Вначале очень легко: нужно будет прыгать на каждый второй столб, но после того как фон начнёт изменяться, нужно сменить частоту прыжков и пройти через портал. Далее тоже самое что и до этого, но нужно нажать лишний раз и опять прыгать каждый второй столб. В конце будет один шип, который надо перепрыгнуть. Уровень пройден!',
            'Секретные монеты',
            [
                'Первая монета находится на 35% на этапе корабля. Во время того как вы переворачиваетесь второй раз, вам надо будет немного подождать, а чуть позже взлететь вниз. Там будет фальшивый блок, в который нужно залететь и собрать монету.',
                'Вторая монета находится на 43% на этапе куба. Во время медленного забирания на верх, вам надо нажать лишний раз, потом ещё раз и ещё, перепрыгнув тройной шип.',
                'Третья монета находится на 68% на втором этапе корабля. Во время того, как фон с красного будет меняться на розовый, вам надо будет опуститься вниз. Другой ориентир: после двух блоков с шипами вниз на одном уровне.'
            ],
            [
                '../../static/img/lavels/Jumper/JMP-C1.png',
                '../../static/img/lavels/Jumper/JMP-C2.png',
                '../../static/img/lavels/Jumper/JMP-C3.png'
            ]
        ],
        'TimeMachine': [
            f'{name}',
            'Гейм-плэй',
            'Уровень начинается с голубого фона, где нужно перепрыгнуть через несколько препятствий. Дальше идёт изменённая гравитация, где нужно перепрыгивать через тройные шипы. Потом фон меняется на синий и после него идёт переворот, где фон меняется на розовый. После идёт корабль. Потом опять кубик, где через некоторое время экран станет чёрным, а разворот экрана станет нормальным. Далее опять переворот и фон становится цветным. Короче говоря, переворот повторяется три раза. И после трёх переворотов начинается второй кораблик, у которого меняется гравитация. Уровень пройден!',
            'Секретные монеты',
            [
                'Первая монета находится на 12% на этапе куба, между двумя столбами, покрытыми шипами. Нужно прыгнуть с края первого столба для её сбора.',
                'Вторая монета находится на 36%. На этапе корабля нужно залететь внутрь большого куба с шипами.',
                'Третья монета находится на 69% на этапе куба. После того, как экран станет чёрным и вы перевернётесь вниз головой, не прыгайте около синего гравитационного портала, затем монета будет собрана. После этого вы должны будете оттолкнуться от двух фиолетовых сфер для успешного приземления.'
            ],
            [
                '../../static/img/lavels/TimeMachine/TM-C1.png',
                '../../static/img/lavels/TimeMachine/TM-C2.png',
                '../../static/img/lavels/TimeMachine/TM-C3.png'
            ]
        ]
    }
